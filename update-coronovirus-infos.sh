...
git status | grep -q 'nothing to commit' || git commit --untracked-files=no -m "Coronavirus FAQ ${STAND}"
...
git status | grep -q 'nothing to commit' || git commit --untracked-files=no -m "Coronavirus ${STAND}"
